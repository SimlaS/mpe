...
if ($countBrakiZasilen>0){
    ...
    foreach($brakiZasilen as $key => $row){
        ...
        if($status=='O'|| $status == 'P'){
            ...
            $poleKomentarza = "<div class='input-group'><select class='form-control' title='Proszê wybraæ...' name='komentarz' id='komentarz_$id'>$optionHtml</select>";
            $poleKomentarza .= "<span class='input-group-btn'>
                                        <a href=".$_SERVER['SCRIPT_NAME']."?option=komentarze&modal data-target='#myModal' class='btn btn-default' type='button' data-toggle='tooltip' title='Edytuj listê'><i class='fa fa-pencil'></i></a>
                                        </span></div>";
        ...
        $text2 = "Anuluj pozycjê nr $id";
        $buttonsHtml = "<input type=submit class='btn btn-success' data-toggle='tooltip' title='{$text}' value='Wyœlij' $disabled  style='margin:5px 10px;'>";
